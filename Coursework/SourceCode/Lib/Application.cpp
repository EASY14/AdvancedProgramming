...
bool Application::_CreateStategyFile()
{
	...
	_ShadowWindowLine("    ~Easy's Game of The Prisoner Dilemma~     ");
	COORD coord;
	coord.X = 0;
	coord.Y = 6;
	...
	if (!creator->CreateStrategyTextFile("StrategyFile", fileNum))
		return false;

	cout << fileNum <<" Strategy text Files Creation has been completed" << endl;
	cout << "Floder name : StrategyFile" << endl;
	//display all file
...
bool Application::_AllStrategyFileCombat()
{
	...
	for (int i = 0; i < (int)winners.size(); i++)
		cout << "winner strategy name : " << winners[i]->first << "  win times : " << winners[i]->second << endl;
...
{
	system("CLS");
	_ShadowWindowLine("    ~Easy's Game of The Prisoner Dilemma~     ", 0);
	COORD coord; coord.X = 0; coord.Y = 6;
	SetConsoleCursorPosition(m_console, coord);
...
bool Application::_ShowMenu()
{
	...
	_ShadowWindowLine("    ~Easy's Game of The Prisoner Dilemma~     ",3);
